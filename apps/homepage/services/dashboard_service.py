...
class DashboardService:
    ...
    def _get_channels_queryset(self):
        ...
        return (
            ...
            .filter(moderators__user=self.user)
            .distinct()
            .annotate(
    ...
    def _build_channels(self, qs):
        ...
        for c in qs[:5]:
            ...
            growth_percent = (
                (growth / max(1, subscribers - growth)) * 100
                if growth else 0
            )
    ...
    def _build_insights(self, qs):
        ...
        if avg_views and avg_views > 5000:
            result.append(InsightDTO(
                text=f"Средние просмотры: {int(avg_views)}",
                type="positive"
            ))

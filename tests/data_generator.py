...
class DataGenerator:
    ...
    def __init__(self, num_of_fixtures=NUM_OF_FIXTURES) -> None:
        ...
        self.rules = {
            "limited": {
                ...
                "url": (
                    ...
                    r"[A-Za-z]{2,6}\/"
                    r"[A-Za-z0-9._~-]{3,50}\."
                    r"(?:apng|avif|gif|jpg|jpeg|jfif|pjp|pjpeg|png|svg|webp|bmp|ico|tiff)"
                ),
                "email": (
                    r"([-!#$%&'*+/=?^_`{}|~0-9A-Za-z]+"
                    r"(\.[-!#$%&'*+/=?^_`{}|~0-9A-Za-z]+)*"
                    r")@"
                    r"([A-Za-z0-9]([A-Za-z0-9\-]{0,61}[0-9A-Za-z])?\."
                    r")*"
                    r"[A-Za-z]{2,}"
                ),
                "datetime": (
        ...
        self.fixtures_generators = {
            ...
            "invalid": {
                ...
                "data_type": None,
                # in future use it for injextions,
                # xss attacks etc
                "validator": None,
            },
        }

    # max len is for fixtures which len is controlled
    # by changeable code in forms etc.
    ...
    def _generate_data(
        ...
        attempts = 0
        # avoid not enough generations when validation is strict -
        # simply make more generations
        max_attempts = self.data_size * max_attempts_multiplier
        # some models require unique values,
        # so we make set() for fields with such data
    ...
    def generate_urls(
        ...
        **kwargs,
        ...
        # sadly, cannot do
        # generate_urls(self, rule: str=self.fixture_generators),
    ...
    def generate_emails(
        ...
        **kwargs,
        ...
        # ensure uniqueness for models
        # like users.User.email (unique=True)
        rule = (
    ...
    def generate_fixtures(self) -> None:
        """
        save_fixture(
            FIXTURE_DIR_PATH/fixturename,
            generate_data,
            generate_invalid_data,
        )
        -> json file with 2 keys ('valid', 'invalid')
        each with data_size num of elems
        """
        for name, cfg in self.fixtures_generators.items():
            if name == "invalid":
                # 'invalid' is generated per-file below;
                # no "valid" set for it
                continue
            valid = cfg["generator"](
                rule=cfg.get("rule"),
    ...
    def save_fixture(
        ...
        valid_data,
        invalid_data,
        fixture_path=FIXTURES_DIR_PATH,
    ) -> None:
        data = {
            "valid": valid_data,

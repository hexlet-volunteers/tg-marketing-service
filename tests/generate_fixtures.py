...
class ModelAndFormFixtureGenerator:
    ...
    why new class and not a script?
    so that can configure in future
    not all models and forms present,
    because some of them require foreign key etc.
    in future, need to connect to present DB
    or make local SQLite, populate it and test via it
    """
    ...
    def model_users_user(self) -> None:
        ...
        required: username (text, unique, 1-150),
        email (email, unique, blank allowed but generate),
        role (text, 1-150)
        optional: first_name (text), last_name (text),
        bio (text, 1-200), avatar_image (url)
        """
        # usernames unique
        ...
        # Invalid: empty username, invalid email,
        # too-long role and bio, non-url avatar image
        invalid = []
    ...
    def model_parser_telegram_channel(self) -> None:
        ...
        required: channel_id (bigint unique), title (text, 1-255),
        participants_count (int), parsed_at (datetime auto)
        optional: username (text, 0-255, can be blank),
        description (text), pinned_messages (json),
        creation_date (datetime), last_messages (json),
        average_views (int)
        """
        ...
        # invalid set: non-int channel_id, empty title,
        # invalid datetime/json, non-int counts
        ...
        for i in range(self.size):
            invalid.append(
                {
                    "channel_id": (
                        invalid_strs[i] if i < len(invalid_strs) else "abc"
                    ),  # should be int
                    "title": "",  # required -> invalid
                    "username": None,
                    "description": None,
                    "participants_count": (
                        invalid_strs[i] if i < len(invalid_strs) else "n/a"
                    ),  # should be int
                    "parsed_at": (
                        invalid_dt[i]
                        if i < len(invalid_dt)
    ...
    def form_user_reg(self) -> None:
        ...
        # password1 (text), password2 (text), email (email),
        # bio (text, can be blank), terms (bool), avatar_image (url)
        first_names = self.gen.generate_text(max_len=50)
        last_names = self.gen.generate_text(max_len=50)
        usernames = self.gen.generate_text(max_len=150, ensure_unique=True)
        ...
        invalid = self._compose(
            {
                ...
                "new_password2": self.gen.generate_text(
                    max_len=12
                ),  # not same len
            }
        )
    ...
    def form_group_create(self) -> None:
        ...
        for i in range(self.size):
            # overly long; not really invalid,
            # but assume like 1000 url is not real
            invalid.append(
                {
                    "name": "",  # required
                    "description": "z" * 1000,
                    "image_url": (
                        invalid_img[i] if i < len(invalid_img) else "invalid"
                    ),
                }
            )
    ...
    def generate_all(self) -> None:
        """
        for now only includes forms and models
        that not require database relations.
        """
